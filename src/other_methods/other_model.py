...
from other_methods.neutex.neutex import make_neutex_train_wrapper_default
from other_methods.layers import FourierFeatEnc, RandomFourierFeatEnc, LinearWithConcatAndActivation, Sine
...
class TextureField(nn.Module):
    ...
    def eval_inf_with_interpolation(self, efuns_triangle, barycentric_coordinates):
        efuns = torch.matmul(barycentric_coordinates.unsqueeze(1), efuns_triangle).squeeze(1)

        res = efuns
        for i in range(len(self.layers)):
            if i == self.skip_layer_idx:
                res = self.layers[i](res, efuns)
            else:
                res = self.layers[i](res)
        return res
